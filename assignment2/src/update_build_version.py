...
def validate_environment():
    ...
    if not source_path:
        raise EnvironmentError("SourcePath environment variable not set")

    if not build_num:
        raise EnvironmentError("BuildNum environment variable not set")

    if not build_num.isdigit():
        raise ValueError("BuildNum must be numeric")
...
def main():
    ...
    update_file(
        os.path.join(src_dir, "SConstruct"), r"point\s*=\s*\d+", f"point={build_num}"
    )
